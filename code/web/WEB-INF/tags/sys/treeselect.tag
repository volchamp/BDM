...
<%@ attribute name="isShowClose" type="java.lang.Boolean" required="false" description="是否显示关闭按钮"%>
...
<script type="text/javascript">
	$("#${id}Button, #${id}Name").click(function(){
        ...
        top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}", "选择${title}", 300, 420, {
            ...
            ajaxData:{selectIds: $("#${id}Id").val()},buttons:{"确定":"ok"${allowClear?',"清除":"clear"':''}${isShowClose?',"关闭":""':''}}, submit:function(v, h, f){
                if (v=="ok"){
