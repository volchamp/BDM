	...
	<script type="text/javascript">
		...
		function saveOrUpdate() {
			...
			if(validateForm()){
				$.ajax({
		            type:"post",
		            url:"${ctx}/codesetmrg/pedningCodeItem/save",
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/codesetmrg/pedningCodeItem/list?id=0&operation=0&processStatus=1">代码集数据列表</a></li>
		<li class="active"><a href="${ctx}/codesetmrg/codeSetData/form?id=${codeSetData.id}">代码集列表<shiro:hasPermission name="codesetmrg:codesetdata:edit">${not empty codeSetData.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="codesetmrg:codesetdata:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="codeSetData" action="${ctx}/codesetmrg/codeSetData/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			<label class="control-label">父代码集编码：</label>
			<div class="controls">
				<sys:treeselect id="parentItemCode" name="parentItemCode" value="${codeSetData.parentItemCode }" labelName="${id}" labelValue="${codeSetData.parentItemCode}"
						title="父代码集编码" url="/codesetmrg/pedningCodeItem/parentCodeTreeData?codeSetNo=${codeSetData.codeSetNo }" cssClass="input-small" allowClear="true" notAllowSelectParent="false" />
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="itemCodeSort" htmlEscape="false" maxlength="18" class="input-xlarge  digits"/>
			</div>
		</div>
		<div class="control-group">
